...
//TODO 全局变量
var emptyReg=/^\s*$/;//是否为空正则对象
var emailReg=/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/;
var phoneReg=/^1\d{10}$/;

function siXin(userId, userName) {
    $('.sxTo_p').html('收件人：'+userName);
}

//TODO 添加或修改url参数函数
function addUrlPar(key,val){
    var loopFlag=false;
    var returnStr="";//最终返回的url字符串
    var addStr="";//后面要追加的字符串
    var locationStr=location.href;//完整url
    var searchStr=location.search;//设置或返回从问号 (?) 开始的 URL（查询部分）
//    var prefixUlrStr=locationStr.replace(searchStr,"");//不带参数的url
    var parArr=[];//参数数组
    //如果没有第二个参数
    ...
    //如果原来没有参数
    if(emptyReg.test(searchStr)){
        returnStr=locationStr.replace(/\?$/,'')+'?'+addStr;
        return returnStr;
    }
    //原来就有参数,但是不包含要添加的这个参数
    if(searchStr.indexOf(key)==-1){
        returnStr=locationStr+'&'+addStr;
        return returnStr;
    }
//    var searchStr="?jum=nihao&name=wohao";
    parArr=function(){
        //debugger;
        var parArr=[];
        var newStr=searchStr.substr(1);
        var newArr=newStr.split('&');
        var itemArr;
        var key='',val='',isEmpty=false;
        var newObj=null;
        for(var i= 0,max=newArr.length;i<max;i++){
            isEmpty=false;
            newObj=new Object();
            itemArr=newArr[i].split('=');
            key= $.trim(itemArr[0]);
            if(itemArr.length==1){
                isEmpty=true;
            ...
            newObj.key=key;newObj.val=val;newObj.isEmpty=isEmpty;
            parArr.push(newObj);
        }
        return parArr;
    }();
    ...
    for(var i= 0,max=parArr.length;i<max;i++){
        if(parArr[i].key==key){
            if(typeof val =='undefined'){
                parArr[i].isEmpty=true;
            }else{
                parArr[i].val=val;
                parArr[i].isEmpty=false;
            }
            loopFlag=true;
            break;
        }
    ...
    if(!loopFlag){
        ...
        returnStr=function(){
            ...
            for(var i= 0,max=parArr.length;i<max;i++){
                ...
                if(parArr[i].isEmpty){
                    returnStr=returnStr+parArr[i].key;
                }else{
                    returnStr=returnStr+parArr[i].key+'='+parArr[i].val;
                }
            }
            return returnStr;
        }();
...
//TODO input输入框在不支持原生placeholder时附加的响应逻辑
var pHInputFocusFn=function(phTip,isFocus){
...
$.fn.setPhTip=function(){
    if(supDetectObj.placeH){
        return;
    }
    this.show().each(function(index,element){
        var _this=$(this);
        var curInput=$(this).prevAll('.sitePHInput');
        $(this).click(function(){
            curInput.focus();
        });
        var curInputDom=curInput.get(0);
        //pHInputFocusFn.apply(curInputDom,[_this,curInput.is(':focus')]);
        curInput.on({
            'focus':function(){
                if(emptyReg.test($(this).val())){
                    $(this).val('');
            ...
            'blur':function(){
                pHInputFocusFn.apply(this,[_this,false]);
            }
        });
    });
}
...
var sitePop;
$(function(){
    sitefixedMask=$('#sitefixedMask');//fixed布局遮罩层

    isLogin=function(){
        var status=$('#siteHTopBox').data('status');
        if(status=="on"){
            return true;
        }else if(status=="off"){
            return false;
    ...
    sitePop={
        showSixin:function(){
            ...
            var sixinBox=$('#siteSiXinBox');//私信弹层盒子
            var sixinText=sixinBox.find('.sxTextArea');//私信输入框
            var errorP=sixinBox.find('.errorP');//错误提示
            var closeA=sixinBox.find('.closeA');//关闭按钮
            var cancelBtn=sixinBox.find('.cancelBtn');//取消按钮
            var submitBtn=sixinBox.find('.submitBtn');//发送按钮
            closeA.add(cancelBtn).click(function(){
                sitefixedMask.add(sixinBox).hide();
            });
            submitBtn.click(function(){
                errorP.text('');
                if(emptyReg.test(sixinText.val())){
                    errorP.text('私信内容不能为空!');
                }else{
                    $.ajax({
                        type: "GET",
                        url: "/message-send",
                        data: {"id":userId,'message':sixinText.val(),'ajax':1,'csrf_token_message':$('#csrf_token_message').val()},
                        dataType: "json",
                        success: function(data){
							sixinBox.hide();
                            if(data.status==1){
                                //alert('发送成功!')
                                $('#csrf_token_message').val(data.csrf_token_message);
								sitePop.conformPop('提示','私信发送成功');
                            }else if(data.status==0){
                                //alert('收信人不存在!');
								sitePop.conformPop('提示',data.info);
                            }
                        }
                    });
                }
            ...
            return function(uId){
                ...
                sixinText.val('');
                errorP.text('');
                sitefixedMask.add(sixinBox).show();
            }
        }(),
        oriPic:function(){
            var initFlag=false;//是否初始化标识
            var isLoading=false;
            var oriPicBg;//查看原图遮罩层
            var oriPicPop;//查看原图弹出层
            var btnBox=null;//按钮外盒子
            var picWrap=null;//大图外盒子
            var preA=null;//前一张
            var nextA=null;//后一张
            var imgObj=null;//大图对象
            var newImg=null;//声明newImg
            var callBkFn;//前一张后一张回调函数
            var maxHeight=500;
            var maxWidth=1000;
            ...
            function initPanel(){
                ...
                var htmlStr1='<div class="siteOriPicBg" id="siteOriPicBg"></div>';

                var htmlStr2='<div class="siteOriPicPop" id="siteOriPicPop">'+
                    '<a href="javascript:;" class="closeA"></a>'+
                    '<div class="picWrap">'+
                    '<img src=""/>'+
                    '</div>'+
                    '<div class="btnBox">'+
                    '<a href="javascript:;" class="oprBtn preA"></a>'+
                    '<a href="javascript:;" class="oprBtn nextA"></a>'+
                    '</div>'+
                    '</div>';
                oriPicBg=$('#siteOriPicBg');
                oriPicPop=$('#siteOriPicPop');
                if(oriPicBg.size()==0 && oriPicPop.size()==0){
                    oriPicBg=$(htmlStr1);
                    oriPicPop=$(htmlStr2);
                    oriPicBg.add(oriPicPop).appendTo(body);
                }
                initFlag=true;//初始化完成

                btnBox=oriPicPop.find('.btnBox');//按钮外盒子
                preA=btnBox.find('.preA');//前一张
                nextA=btnBox.find('.nextA');//后一张
                picWrap=oriPicPop.find('.picWrap');
                imgObj=picWrap.find('img');
                oriPicPop.find('.closeA').click(function(){
                    closeFn();
                });
                oriPicBg.click(function(event){
                    var tarObj=$(event.target);
                    if(tarObj.attr('id')!='siteOriPicPop' && tarObj.parents('#siteOriPicPop').size()==0){
                        closeFn();
                    }
                });
                //前一张和后一张绑定事件
                preA.add(nextA).on('click',function(){
                    var direction;
                    if($(this).hasClass('preA')){
                        direction="pre";
                    }else if($(this).hasClass('nextA')){
                        direction="next";
                    }
                    var srcStr=callBkFn(direction);
                    //alert(imgStr);
                    sitePop.oriPic(srcStr,'switch',true);
                });
            }

            /**
             * 面板弹出之前重置界面操作
             * @param {String} srcStr 图片src字符串
             * @param {String} method 如果值为“pop"表示是弹出，如果是"switch"表示是切换
             */
            function popResetPanel(method,par02){

                if(typeof par02=='undefined' || par02==false){
                    btnBox.hide();
                }else if(typeof par02=="function"){
                    callBkFn=par02;
                    btnBox.show();
                }else if(par02==true){
                    btnBox.show();
                ...
                oriPicBg.add(oriPicPop).show();
            }
            /**
             * 图片加载出来后调整位置
             */
            function popAdjustPanel(srcStr){
                //console.log(srcStr);
                //debugger;
                ...
                if(imgWidth>maxWidth || imgHeight>maxHeight){
                    if(imgWidth>imgHeight*maxRadio){
                ...
                if(imgHeight<300){
                    imgObj.css({
                        'padding-top':(300-imgHeight)/2
                    });
                }
                if(imgWidth<300) {
                    imgObj.css({
                        'padding-left': (300 - imgWidth) / 2
                    });
                }

                oriPicPop.css({
                    'margin-top':imgHeight>300?-(imgHeight+30)/2:-165,
                    'margin-left':imgWidth>300?-(imgWidth+30)/2:-165
                });
                picWrap.removeClass('bg');
                imgObj.attr('src',srcStr).css({
                    'display':'block',
                    'opacity':1
                });
                isLoading=false;
            ...
            return function(srcStr,method,par03){
                ...
                newImg=new Image();//赋值newImg

                if(!initFlag){
                    initPanel();
                }

                popResetPanel(method,par03);

                newImg.src=srcStr;
                //debugger;
                if(newImg.complete){
                    popAdjustPanel(srcStr);
                    return;
                }
        ...
        conformPop:function(){
            var initFlag=false;//是否初始化标识
            var popBox=null;//弹层外盒子
            var h3=null;
            var popCont=null;//弹层内容盒子
            var okBtn=null;//确认按钮
            var cancelBtn=null;//取消按钮
            var closeA=null;//关闭按钮
            var btnNum=1;

            //参数
            var title;//标题
            var tip;//提示文案
            var type;//文案类型，text表示纯文本，html表示是html代码
            var okObj={//确认按钮回调
                'name':'',
                'fn':null
            };
            var cancelObj={//取消按钮回调
                'name':'',
                'fn':null
            };
             ...
             */
            function closeFn(){
                popBox.add(sitefixedMask).hide();
            }

            /**
             * 参数调整
             */
            function argumentReset(arg){
                btnNum=1;
                title="标题";
                tip="提示内容";
                type='text';
            ...
            function initPanel(){
                ...
                var htmlStr='<div class="siteConformPop" id="siteConformPop">'+
                    '<a href="javascript:;" class="closeA"></a>'+
                    '<h3 class="siteConform_h3"></h3>'+
                    '<div class="popContBox">'+
                    '</div>'+
                    '<div class="popBtnBox siteIlB_box">'+
                    '<input type="button" class="okBtn" value="确定"/>'+
                    '<input type="button" class="cancelBtn" value="取消"/>'+
                    '</div>'+
                    '</div>';
                popBox=$('#siteConformPop');
                if(popBox.size()==0){
                    popBox=$(htmlStr);
                ...
                popCont=popBox.find('.popContBox');
                h3=popBox.find('.siteConform_h3');
                closeA=popBox.find('.closeA');
                okBtn=popBox.find('.okBtn');//确定按钮
                cancelBtn=popBox.find('.cancelBtn');//取消按钮
             ...
             *接受参数后调整面板
             */
            function popAdjustPanel(){
                h3.text(title);
                if(type=="text"){
                    popCont.html('<p class="tip_textP">'+tip+'</p>');
                }else if(type=="html"){
                    popCont.html(tip);
                }
                popBox.css({
                    'margin-top':-popBox.height()/2
                });
                okBtn.val(okObj.name);
                cancelBtn.val(cancelObj.name);
                okBtn.add(cancelBtn).show();
                if(btnNum==1){
                    cancelBtn.hide();
                }
				//debugger;
                popBox.add(sitefixedMask).show();
            }

            return function(title,tip,type,okObj,cancelObj){
                if(!initFlag){
                    initPanel();
                }
                argumentReset(arguments);
                popAdjustPanel();
            }
        }(),
        //通用大文本弹框
        popUp:function(){
             ...
             */
            function closeFn(){
                popBox.add($('#popUpBoxFixMask')).hide();
            }
            ...
            function initPanel(){
                var body=$(document.body);
                var htmlStr='<div class="popUpBox " id="sitePopupBox">'+
                         '<div class="sxHeader"><span class="popUpTitle"></span><a href="javascript:;" class="closeA btn_ALink"></a></div>'+
                         '<div class="sxCont">'+
                         '<div class="sxText popUpContent"></div>'+
                         '<div class="sxBtnBox siteIlB_box">'+
                         '    <input type="button" class="okBtn" value="确定">'+
                         '</div>'+
                         '</div>'+
                         '</div>'+
                         '<div class="popUpBoxFixMask" id="popUpBoxFixMask"></div>';
                popBox=$('#sitePopupBox');
                if(popBox.size()==0){
                    popBox=$(htmlStr);
                    popBox.appendTo(body);
                ...
                popCont=popBox.find('.popUpContent');
                h3=popBox.find('.popUpTitle');
                closeA=popBox.find('.closeA');
                okBtn=popBox.find('.okBtn');//确定按钮
                cancelBtn=popBox.find('.cancelBtn');//取消按钮
                okBtn.on('click',function(){
...
//                okBtn.add(cancelBtn).show();
				...
				//debugger;
                popBox.add($('#popUpBoxFixMask')).show();
            }
    ...
    $(window).scroll(function(){
        try{
            goTopScrollFn(SiteGoTopBox);//返回顶部按钮
        }catch(ex){
    ...
    //TODO 意见反馈
    (function(){
        var placeHFlag='placeholder' in document.createElement('input');//判断是否支持placeholder
        var yjfkWrap=$('#yjfkWrap');//意见反馈弹出层外盒子
        var siteFixMask=$('#siteFixMask');//Fix定位遮罩层
        var yjfkInputS=yjfkWrap.find('.yjfkInput,.yjfkTextArea');//输入框
        var placeHTextS=yjfkWrap.find('.yjfk_placeHText');//输入框提示语span盒子
        var yjfkTextArea=$('#yjfkTextArea');//意见反馈内容盒子
        var errorP=$('#yjfk_errorP');//错误提示p标签
        //TODO 如果不支持placeholder，代码模拟如果不支持placeholder效果
        if(!placeHFlag){
            placeHTextS.show();
            var hidePlaceHText=function(event){
                var curVal=$(this).val();//当前输入框的内容
                var nextHText=$(this).next('.yjfk_placeHText');//当前提示span
                var eventType=event.type;//事件类型
            ...
            yjfkInputS.on('focus blur',function(event){
                hidePlaceHText.call(this,event)
            });
            placeHTextS.click(function(){
                $(this).prev('.yjfkInput,.yjfkTextArea').focus();
            });
        }
        //TODO 意见反馈内容提交
        $('#yjfkSubmit').click(function(){
            if(/^\s*$/.test(yjfkTextArea.val())){
                errorP.text('请输入意见反馈内容！').css('visibility','visible');
                return;
            }else{
                errorP.css('visibility','hidden');
            }
            //执行后台ajax提交
            var ajaxurl = '/corp-feedback';
            $.ajax({
                 url: ajaxurl,
                 dataType: 'json',
                 data:{
                     'content':yjfkTextArea.val(),
                     'contact':$('.yjfkInput').val()
                 },
                 type: 'POST',
                 success: function(ajaxobj){
                     if(ajaxobj.code == 1){
                         alert(ajaxobj.message);
                         siteFixMask.add(yjfkWrap).hide();
                     }
                     else if(ajaxobj.code == 0){
                         alert(ajaxobj.message);
                     }
                 }
        ...
        $('#yjfkCancel,#yjfk_closeA').click(function(){
            siteFixMask.add(yjfkWrap).hide();
        });
        $('#RS_yjfkBox').click(function(){
            //先进行面板重置
            yjfkInputS.val('').blur();
            errorP.css('visibility','hidden');
            siteFixMask.add(yjfkWrap).show();
        });
        //详情右侧意见反馈
        $('#RS_yjfkBox_right').click(function(){
            //先进行面板重置
            yjfkInputS.val('').blur();
            errorP.css('visibility','hidden');
            siteFixMask.add(yjfkWrap).show();
        ...
        $(document.body).on('click', '.yjfk-button', function(){
            //先进行面板重置
            yjfkInputS.val('').blur();
            errorP.css('visibility','hidden');
            siteFixMask.add(yjfkWrap).show();
...
function eq_jump(userId, token, url, not_jump){
	var jump = "http://guquan.yuanshihui.com/about/login?userId=" + userId + "&userToken=" + token + "&url=" + encodeURIComponent(url);
	if(!not_jump)
...
// 搜索弹出效果
$(function(){
    $('.newsoso .newso-icon').click(function() {
        if($('.newso-box').width()==0){
          $('.newso-box').width(0).css({'margin-left':182,border:'1px solid rgba(255,255,255,0.1)'}).animate({width: 182,'margin-left':0,border:'none'}, 200)  
          $('.others .newso-box').width(0).css({'margin-left':182,border:'1px solid #e0e0e0'}).animate({width: 182,'margin-left':0,border:'none'}, 200)
          return false;
        }
        if($('.newso-box').width()==182){
    ...
    $('.indextop #searchInput').focus(function(){
        if($(this).val()==$(this)[0].defaultValue){
            $(this).css('color',"#fff");
            $(this).val("");
        }
    });
    $('.indextop #searchInput,.others #searchInput').blur(function(){
        if($(this).val()==""){
            $(this).css('color',"#aaa");
             $(this).val($(this)[0].defaultValue);
        }
    });
    $('.others #searchInput').focus(function(){
        if($(this).val()==$(this)[0].defaultValue){
            $(this).css('color',"#333");
            $(this).val("");
        }
        ...
        // 右侧悬浮固定
    $(window).scroll(function() {
        $('.xqgotop .gotop').css('display','block');
        if($(window).scrollTop()==0){
           $('.xqgotop .gotop').hide();
        }
    });
    ...
    function gotop(){
     ...
     $('.float-wrap').css('right',oright+'px')
    }
...
function InputHinter(options){
	...
	this.encodeKw = encodeKw;
	
	var url = "/search-suggestion/kw-";
	var $target = $("#" + options.id);
	var $container = $("#" + options.container);
	var $hinter = $container.find(".input-hinter");
	var $form = $("#" + 'searchForm');
	var sIndex = -1;
	var lock = false;
	var last_input = {kw : '', time : new Date(), done:true};
	
	function onTxtChange(){
		var kw = $target.val();
		var d = new Date();
		if(kw !== last_input.kw)
			last_input = {kw:kw, time : d, done:false};
	}
	...
	function keyUp(e){
		if(getKeyCode(e) === 40){ // Down
			selectionNext();
			selectionRender();
		}else if(getKeyCode(e) === 38){ // Up
			selectionPrev();
			selectionRender();
		}
	...
	function enter(){
		if(sIndex === -1){
			window.location.href = '/search/kw-' + encodeKw($.trim($target.val()) || $target.attr('placeholder'));
		}else{
			$rows = $hinter.find(".row");
			var r = $rows[sIndex];
			var href = $(r).children("a").attr('href');
	...
	$target.on("blur", function(){
		//last_input = {kw : '', time : new Date(), done:true};
		setTimeout(function(){
			$hinter.hide();	
		}, 300);
	...
	function consumer(){
		if(last_input.kw){
			var diff = (new Date() - last_input.time);
			if(diff > options.input_threshold && !last_input.done){
				last_input.done = true;
				onKwChange(last_input.kw);
	...
	var I = setInterval(consumer, options.delay);
	
		
	function onKwChange(kw){
		$.getJSON(url + encodeKw(kw), function(result) {
			if (result.errcode == 0) {
				$hinter.empty();
				...
				if (hits.length > 0) {
					for (var i = 0; i < hits.length; i++) {
						var hit = hits[i];
						var row = "<div class='row'><a href='" + hit.url + "' target=\"_blank\">" + hit.name + "</a></div>";
						$hinter.append(row);
					}
					$hinter.show();
					selectionInit();
	...
	function encodeKw(kw) {
		kw = encodeURIComponent(kw || '').replace('-', '%252D');
		return kw;
	};
